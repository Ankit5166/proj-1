...
class VendingMachine(object):
    ...
    PRODUCTS = {"can pop(100\u00A2)": ("can pop",100),
                "chips(150\u00A2)": ("chips",150),
                "bar(75\u00A2)": ("bar",75),
                "candy(50\u00A2)": ("candy",50),
                "suprise(225\u00A2)": ("chocolate milk", 225)}
...
if __name__ == "__main__":
    ...
    total_amount_text = sg.Text("Total Amount: $0.00", font=("Helvetica", 18))
    select_col.append([total_amount_text])
    ...
    while True:
        ...
        if event != '__TIMEOUT__':
            log((event, values))
            if event in VendingMachine.COINS:
                coin_name,coin_value = VendingMachine.COINS[event]
                print(f"Coin entered: {coin_name} (${coin_value/100:.2f})")
        if event in (sg.WIN_CLOSED, 'Exit'):
            break
        if event != '__TIMEOUT__':
            print(f"Event:{event},Current state:{vending.state.name}")
        
        vending.event = event
        vending.update()
        
        total_amount_text.update(f"Total Amount: ${vending.amount/100:.2f}")
